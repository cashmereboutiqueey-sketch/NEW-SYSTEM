...
| ~~**VAT treatment of Shopify retail prices**~~ | **Resolved 2026-08-07 by the owner: the business is not VAT-registered and does not charge VAT yet.** Sales post at full value against the `VAT-EXEMPT` rate; no output VAT is recognised. The versioned `TaxRate` mechanism and the `splitTaxInclusive` helper stay in place so registration is a configuration change, not a rewrite. See `docs/DECISIONS.md` D-001. |
| ~~**Factory margin basis**~~ | **Resolved 2026-08-07 by the owner: the margin applies to full factory cost including materials** — `transfer_price = (material_cost + cmt_cost) × (1 + factory_margin)`, which is what the schema and specs already assume. No change required. See `docs/DECISIONS.md` D-002. |
